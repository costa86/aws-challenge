...
class Triangle(BaseModel):
    lenghts: list[PositiveFloat] = Field(
        max_items=TRIANGLE_SIDES, min_items=TRIANGLE_SIDES
    )
...
def get_response(success: bool = True, message: str = "ok") -> dict:
...
def lambda_handler(event, context):
    try:
        ...
        triangle.save()
        return get_response(True, triangle.get_type())

    except ValidationError as e:
        return get_response(False, f"validation failed: {e}")

    except Exception:
        return get_response(False, "could not process request")
